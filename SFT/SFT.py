...
tokenizer = AutoTokenizer.from_pretrained('/FineMedLM-s2-lr5e-6/checkpoint-506', trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained('/FineMedLM-s2-lr5e-6/checkpoint-506') # device_map='auto',自动分配模型到多个 GPU
...
args = SFTConfig(
    output_dir="FineMedLM-s3-lr5e-6",
    per_device_train_batch_size=8,
    gradient_accumulation_steps=4,
    logging_dir="/logs", # TensorBoard日志文件保存路径
...
trainer.train()
# trainer.save_model(output_dir="/FineMedLM-lr1e-5/final")
# tokenizer.save_pretrained("/FineMedLM-lr1e-5/final")
