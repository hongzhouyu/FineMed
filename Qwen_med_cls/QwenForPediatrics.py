...
def process_output(i, output):
    global PediatricInternal_count, PediatricSurgery_count, OtherDepartments_count, unknown_count
    generated_text = output.outputs[0].text
    # print(f"第 {i} 个输出：{generated_text}\n")
    if generated_text == 'Pediatric Internal Medicine':
        with lock:  # 加锁以确保线程安全
            PediatricInternal_list.append(i)
            PediatricInternal_count += 1
    elif generated_text == 'Pediatric Surgery':
        with lock:  # 加锁以确保线程安全
            PediatricSurgery_list.append(i)
            PediatricSurgery_count += 1
...
# 初始化共享变量
PediatricInternal_list = []
PediatricSurgery_list = []
...
PediatricInternal_count = 0
PediatricSurgery_count = 0
...
ids = 0
# 保存为 .npy 文件
np.save(f'/home/ubuntu/disk/medical_data/Pediatrics/PediatricInternal/PediatricInternal_list_{ids}.npy', PediatricInternal_list)
np.save(f'/home/ubuntu/disk/medical_data/Pediatrics/PediatricSurgery/PediatricSurgery_list_{ids}.npy', PediatricSurgery_list)

np.save(f'/home/ubuntu/disk/medical_data/Pediatrics/OtherDepartments/OtherDepartments_list_{ids}.npy', OtherDepartments_list)
np.save(f'/home/ubuntu/disk/medical_data/Pediatrics/unknown/unknown_list_{ids}.npy', unknown_list)

# 保存为 JSON 文件
with open(f'/home/ubuntu/disk/medical_data/Pediatrics/unknown/unknown_output_{ids}.json', 'w') as f:
